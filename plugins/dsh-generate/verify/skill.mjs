...
check('it carries the split run control rule, so a mode-declaring provider keeps its segment', /A provider that declares run modes gets the split run control/.test(designShipped) && /never draw a\s+segment that opens an empty list/.test(designShipped))
