...
check('it carries the two-button run rule, so a mode-declaring provider keeps its choice', /A provider that declares run modes gets two buttons, not a split one/.test(designShipped) && /never draw a control that opens an empty list/.test(designShipped))
check('it carries the sparkle-menu rule: authored skills, greyed until they are written', /A one-press fill is a sparkle over a menu, and it is authored data/.test(designShipped) && /`ui\.presets: \[\{ label, doors \}\]`/.test(designShipped) && /`disabled: true`/.test(designShipped))
