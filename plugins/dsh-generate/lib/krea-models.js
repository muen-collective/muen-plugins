 ...
 *
 * THE DOORS ARE KREA'S OWN, READ FROM KREA'S OPENAPI (2026-09-23). The founder's three SDK
 * calls — `image/krea/krea-2/medium-turbo`, `image/krea/krea-2/medium` and
 * `image/krea/krea-2/large` — resolve to the three `POST /generate/image/krea/krea-2/*`
 * endpoints. `additionalProperties: false` and `required: [prompt, aspect_ratio, resolution]`
 * are the API's, the `-100..100` slider bounds and the `0.99` denoising default are the
 * API's, and the creativity default is the endpoint schema's own `low` — on all three Krea 2
 * pages, including `krea-2/medium` and `krea-2/large`, where the overview prose calls
 * `medium` the default. The schema wins because the surface always sends the value it shows,
 * so the disagreement is recorded on the door rather than resolved silently. Nothing here is
 ...
 * blurbs are those cards' descriptions, so a card here says what Krea says it says. The
 * per-generation prices on those cards (2, 9 and 20 coins) are deliberately not carried: the
 * provider row already names the funding (a prepaid USD API balance), and this catalogue has
 * no price field.
 *
 * EACH MODEL IS A WHOLE OBJECT, doors included, because a profile replaces one of these by
 * name and a half-inherited model would be a model nobody wrote. The three Krea 2 bodies are
